...
\newcommand{\hmwkDueDate}{Wednesday,\ February\ 15,\ 2017} % Due date
\newcommand{\hmwkClass}{CEEN\ 8886: Wireless Security} % Course/class
\newcommand{\hmwkClassTime}{4:00pm} % Class/lecture time
\newcommand{\hmwkClassInstructor}{Dr. Yi Xian} % Teacher/lecturer
\newcommand{\hmwkAuthorName}{Kelly Boswell} % Your name
...
128-bit key of all ones, 1) the original contents of \emph{State} is displayed; 2) the
value of \emph{State} after the initial AddRoundKey is displayed; 3) the value of \emph{State}
after SubBytes is displayed; and 4) the value of \emph{State} after ShiftRows is displayed.

However, since the meaning of `the 128-bit key of all ones` is ambiguous, several different
different values of each are displayed for different key values.
%\begin{verbatim}
%\end{verbatim}
